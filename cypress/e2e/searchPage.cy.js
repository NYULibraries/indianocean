describe('Search page visit', ()=>{
	describe('Text content load', () => {
		it('Should load text content', ()=>{
			cy.visit('/search');
			cy.get('.page-title', {timeout: 3000}).contains('Browse titles')
	...
	describe('Check filter',()=>{
		it('Should check items after being sorted by author',()=>{
			...
			cy.url().should('include','http://localhost:3000/search?q=*:*&page=1&sortField=ss_sauthor&sortDir=asc')
			cy.wait(2000)
			let authors = []
			cy.get('.flex-container').find('span.md_author:nth-child(2)').each(($author)=>{
		...
		it('Should check items after being sorted by subject',()=>{
			...
			cy.url().should('include','http://localhost:3000/search?q=*:*&page=1&sortField=ss_ssubject&sortDir=asc')
			cy.wait(2000)
			let subjects = []
			cy.get('.flex-container').find('a.md_subject').each(($subject)=>{
		...
		it('Should check items after being sorted by place',()=>{
			...
			cy.url().should('include','http://localhost:3000/search?q=*:*&page=1&sortField=ss_publocation&sortDir=asc')
			cy.wait(2000)
			let pubs = []
			cy.get('.md_publisher').find('span:nth-child(2)').each(($pub)=>{
				pubs.push($pub.text())
		...
		it('Should check items after being sorted by title',()=>{
			cy.get('#browse-select').select(0)
			cy.url().should('include','http://localhost:3000/search?q=*:*&page=1&sortField=ss_longlabel&sortDir=asc')
			cy.wait(2000)
			let titles = []
			cy.get('.md_title').find('a:nth-child(1)').each(($title)=>{
	...
	describe('Check search',()=>{
		it('Should type something into the search,submit, and check url', ()=>{
			...
			cy.get('form').submit()
			cy.url().should('eq','http://localhost:3000/search?q=abc')
